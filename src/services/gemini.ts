...
export async function findRelevantMeetings(query: string, meetings: { id: string, title?: string, name?: string, rawMinutes?: string, minutes?: string }[]) {
  try {
    const meetingContext = meetings.map(m => ({
      ...
      snippet: (m.rawMinutes || m.minutes || "").substring(0, 500) // Send snippets to save tokens
    }));
...
${minutes}`;
    ...
    if (metadata?.attachments && metadata.attachments.length > 0) {
      ...
      metadata.attachments.forEach((att: any, idx: number) => {
...
export async function askAboutMeetings(question: string, meetings: any[]) {
  try {
    const context = meetings.map(m => ({
      ...
      time: m.time,
      duration: m.duration,
      attendees: m.attendees?.map((a: any) => `${a.name} (${a.role})`).join(", ") || "",
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      config: {
        systemInstruction: "You are a professional meeting analysis assistant. Use the provided list of meetings to answer the user's question. Be direct, clear, and refer to specific meetings. Format your response beautifully in Markdown.",
      },
...
Action Items: ${JSON.stringify(previousMeeting.actionItems || [])}`;
    ...
    // Clean up markdown block if model didn't obey
...
export async function askCorporateMemory(
  ...
  try {
    ...
    let text = response.text || "I was unable to retrieve a response from corporate memory.";
